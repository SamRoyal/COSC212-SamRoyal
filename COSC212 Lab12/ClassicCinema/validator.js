...
function checkNameDomain(textValue){
...
function messageDisplay(messages){
    document.getElementById("messages1").innerHTML = "<span style = 'color: red;'>"+messages+"</span>";
}

function checkAddressValidation(deliveryName, deliveryEmail,deliveryAddress1,deliveryCity,deliveryPostcode,messages) {
    if (!checkNotEmpty(deliveryName)) {
        messages.push("Must enter a delivery name");
    } else if (!checkNameDomain(deliveryEmail)) {
        messages.push("Must enter a valid email");
    } else if (!checkNotEmpty(deliveryAddress1)) {
        messages.push("Must enter a delivery address");
    } else if (!checkNotEmpty(deliveryCity)) {
        messages.push("must enter a delivery city");
    } else if (!checkDigits(deliveryPostcode) || !checkLength(deliveryPostcode, 4, 4)) {
        messages.push("Must enter a valid postcode");
    }
}
...
function validateCheckout() {
    var messages, cardType, cardNumber, cardMonth, cardYear, cardValidation, errorHTML,deliveryName, deliveryEmail,deliveryAddress1,deliveryCity,deliveryPostcode;
    ...
    checkAddressValidation(deliveryName,deliveryEmail,deliveryAddress1,deliveryCity,deliveryPostcode, messages);
    ...
    if (messages.length === 0){
        ...
        document.getElementById("p1").innerHTML ="THANK YOU FOR YOUR ORDER";

    } else {

        console.log(JSON.stringify(messages));
        messageDisplay(JSON.stringify(messages));
